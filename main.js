...
Метод filter() приймає функцію, яка перевіряє кожен елемент масиву, якщо функція повертає true, елемент додається (push)
до нового масиву.
...
Якщо функція викликається як метод об'єкта (через посилання на об'єкт та крапку(dot натацію), то this буде вказувати на
цей об'єкт.

Отже, коли ми викликаємо user.showName(), функція showName має доступ до всіх властивостей об'єкта user через this.
...
Коли ти викликаєш функцію в global scope (не в об'єкті), контекст за замовчуванням — це глобальний об'єкт (в браузерах
це window).

window — це глобальний об'єкт в рамках якого запускається JS і будь які функції в JS запускаються в рамках window, або
в рамках об'єктів, які ми створюємо. І ось ці об'єкти в рамках яких викликається функція і є контекст виконання.
...
const user = {
    ...
    isAdmin: true,
    userData: [1, 2, 3, 4, 5, 6, 7, 8, 9],
    showContext //посилання на функцію
};

user.showContext(); //тіло об'єкту user - бо функція була викликана через dot notation,тобто в рамках об'єкту user
...
const person = {
    name: "Kate",  // з властивістю name поганий приклад, бо у window є вбудована властивість браузера window.name
    age: 23,
    showAge: function() {
        console.log(this.age); // this посилається на об'єкт person
    }
};

const showAgeFunc = person.showAge; // копіюємо посилання на функцію — тепер greetFunc це просто функція, яка вже не
                                   // прив'язана до об'єкта person. This тепер вказує на глобальний контекст.

showAgeFunc(); // викликаємо функцію - undefined, оскільки this тепер вказує на глобальний об'єкт
              // ФУНКЦІЯ ВИКЛИКАЄТЬСЯ НЕ ЯК МЕТОД
...
const person = {
    name: "Kate",
    age: 23,
    showAge: () => console.log(this.age) // `this` вказує на глобальний об'єкт
};

person.showAge(); // Виведе undefined, тому що стрілкова функція не запам'ятовує контекст
...
user.showAlsoName(); //Hello! My name is . I'm undefined years old.  (бо window.name: "")


Стрілкові функції не мають власного this. Вони успадковують this від зовнішньої функції або контексту, де були створені.

Тобто стрілкова функція ігнорує об'єкт, у якому вона була викликана, і не прив'язує this до цього об'єкта.
Вона дивиться вище, на контекст, де цей об'єкт був створений.
...
const user = {
     ...
     isAdmin: true,
     userData: [1, 2, 3, 4, 5, 6, 7, 8, 9],
     showContext // додаємо функцію showContext як метод об'єкту user
}
...
Це методи в JS, які дозволяють змінювати контекст виклику функції (значення this усередині функції).
Вони або викликають нашу функцію на якомусь з об'єктів, або прив'язують функцію до якогось об'єкта.


1. call() - викликає функцію одразу на різних об'єктах, передаючи їй конкретний контекст (this) та аргументи
...
2. apply() - подібний до call(), але аргументи передаються масивом
showContext.apply(user); //object user

--------------------------

3. bind() - не викликає функцію одразу, а створює нову функцію з прив'язаним контекстом
Прив'язую нашу функцію до якогось конкретноо об'єкта

const newFunction = showContext.bind(userData);
newFunction(); //object userData


❗В дійсності КОНТЕКСТ НЕ МОЖЕ БУТИ ВТРАЧЕНИЙ АБО ПІДМІНЕНИЙ, ми просто викликаємо функцію в рамках різних об'єктів ❗
...
Це техніка в JS, в якій кожен метод повертає сам об'єкт, що дозволяє викликати методи підряд без проміжних змінних
