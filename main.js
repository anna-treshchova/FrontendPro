...
async function apiAddTodoEl(title) {
...
async function apiToggleTodoEl(id, completed) {
...
async function apiDeleteTodoEl(id) {
    const res = await fetch(`${API_URL}/${id}`, {
        method: "DELETE",
    })
...
todoForm.addEventListener("submit", async (e) => {
    ...
    try {
        const todoEl = await apiAddTodoEl(title);  // бо apiToggleTodoEl - це async функція
        // async функція завжди повертає проміс, перед промісом ми завжди ставимо await, щоб дочекатись його виконання
...
todoList.addEventListener("change", async (e) => {
    if (e.target.classList.contains("toggle-checkbox")) {
        ...
        try {
            await apiToggleTodoEl(todoId, checked)
            e.target.parentElement.classList.toggle("done");
        } catch (err) {
            console.log(err.message)
        }
